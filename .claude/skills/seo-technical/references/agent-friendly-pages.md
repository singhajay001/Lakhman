...
`"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run render_page.py <URL> --mode auto --a11y-tree --json` loads the
page headlessly and captures Chromium's full accessibility tree through CDP.
Use `"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run agent_ux_check.py <URL> --json` for the bounded Agent-UX
