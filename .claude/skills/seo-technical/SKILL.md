...
- XML sitemap: run `"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run sitemap_discovery.py <url> --json`; require a
...
"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run agent_ux_check.py https://example.com --json
```
...
`"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run render_page.py <url> --a11y-tree --json`.
...
If Google API credentials are configured, use `"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run pagespeed_check.py <url> --json` for real PSI + CrUX field data (replaces lab-only CWV estimates), `"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run crux_history.py <url> --json` for 25-week CWV trends, and `"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run gsc_inspect.py <url> --json` for real indexation status per URL.
