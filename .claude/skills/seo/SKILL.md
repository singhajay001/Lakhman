...
**Runtime:** Run bundled Python tools through `"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run <script.py>`. Plugin
...
to repair dependencies. Execute `"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" setup`, report core and Chromium
status separately, and do not fall back to global or user package installation.
For diagnosis, execute `"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" doctor --json`; its output intentionally omits
absolute paths and environment values. If any `"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run` command reports
...
3. If Google API credentials detected (`"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run google_auth.py --check`), also spawn seo-google agent
...
6. If backlink APIs detected (`"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run backlinks_auth.py --check`), also spawn seo-backlinks agent
...
10. If drift baseline exists for this URL (`"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run drift_history.py <url>`), also spawn seo-drift agent
...
After any analysis command completes, offer to generate a PDF report via `"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run google_report.py`.
