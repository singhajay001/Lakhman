...
Configure with: `"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run dataforseo_costs.py config --mode threshold --threshold 0.50 --daily-limit 10.00`
...
1. Run `"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run dataforseo_costs.py check <endpoint> [--count N]`
...
5. After call completes, log: `"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run dataforseo_costs.py log <endpoint> <cost>`
