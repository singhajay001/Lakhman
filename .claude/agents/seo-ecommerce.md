...
2. Analyze product pages with `"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run render_page.py <URL> --mode auto` and `"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run parse_html.py <URL>`
...
4. If DataForSEO credentials available, fetch marketplace data via
   `"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run dataforseo_merchant.py`

## Cost Guardrails

Before ANY DataForSEO Merchant API call:
```bash
"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run dataforseo_costs.py check <endpoint>
```
...
```bash
"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run dataforseo_costs.py log <endpoint> <actual_cost>
```
