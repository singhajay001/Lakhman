...
```bash
"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run capture_screenshot.py URL --all --output screenshots/
"$CLAUDE_PROJECT_DIR/.claude/seo-toolkit/bin/claude-seo" run render_page.py URL --mode auto --a11y-tree --json
```
