...
/**
 * One queue per process. Without REDIS_URL it falls back to the in-memory queue and
 * says so loudly — an in-memory queue loses work on restart, which is fine for a
 * first look at the app and not fine for anything else.
 */
